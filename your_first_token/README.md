...
Don't worry. Metadata is mutable by default. This means you can always just change it later (assuming you don't go out of your way to revoke that permission). If you want to later update the metadata (after first initialization) using the metadata.bat script, you will first need to edit it as following:
...
One way to do that is to explore the [snapshot and airdrop tool](../airdrop) in this repo to achieve a wide initial distribution by airdropping the token to the top 20 holders of another existing token whose community you really like, without having having to create your own community and then distribute the tokens manually from scratch.
